...
class Util(object):
    ...
    def getLinkStatus(self, url):
        linkResponse = requests.get(url=url)
        return linkResponse.status_code

    def verifyLink(self, url):
        linkStatusCode = self.getLinkStatus(url=url)
        if linkStatusCode == 200:
            logging.info(msg=' '.join([url, str(linkStatusCode)]))
            return True
        else:
            logging.info(msg=' '.join([url, str(linkStatusCode)]))
            return False